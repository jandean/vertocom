...
;$(document).ready(function() {
  ...
  $('select.selectize').selectize({ create: false });
  
  $('.nav-first .navbar-nav > li a').click(function() {
    // Act on the event        
    var navtrigger = $(this).attr('id');
    
    $(this).parent('li').siblings('.active').removeClass("active");
    $(this).parent('li').addClass("active");
        
    if($('.topbar .nav-sub').hasClass('active')){
      $('.topbar .nav-sub.active').slideUp("slow", function(){
        $(this).removeClass("active");
      });
                  
      if(!$('.topbar #sub'+ navtrigger).hasClass("active")){
        $('.topbar #sub'+ navtrigger).slideDown("slow").addClass("active");
      }
      // console.log("active subnav present");
    }else{
      // console.log("active subnav none");
     $('.topbar #sub'+ navtrigger).slideDown("slow").addClass("active"); 
    }        
  });
  
  
  //animate scroll from http://www.1stwebmagazine.com/jquery-scroll-to-anchor-point
